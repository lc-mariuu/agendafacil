...
;(function () {
  ...
  var ag2State = {
    ...
    ordemAsc: true,
    dropdownAberto: null,                     // id do item com dropdown aberto
    verMaisAberto: false,
    profFiltro: 'todos',
  };
  ...
  function avatarColorAg2(nome) {
    ...
    var h = 0;
    for (var c of (nome || 'A')) h = ((h << 5) - h) + c.charCodeAt(0);
    return paletas[Math.abs(h) % paletas.length];
  }
 
  function statusClassAg2(status) {
  ...
  function statusLabelAg2(status) {
    var map = { confirmado: 'Confirmado', pendente: 'Pendente', cancelado: 'Cancelado', concluido: 'Concluído' };
    return map[status] || status;
  }
 
  function fmtDataExtenso(dateStr) {
    if (!dateStr) return '';
    ...
    return diasNome[d.getDay()] + ', ' + d.getDate() + ' de ' + mesesNome[d.getMonth()];
  }
 
  function fmtMesAno(year, month) {
    var meses = ['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'];
    return meses[month] + ' ' + year;
  }
 
  function ag2Toast(msg, cor) {
    var t = document.getElementById('ag2-toast-global');
    if (!t) {
      t = document.createElement('div');
      t.id = 'ag2-toast-global';
      t.className = 'ag2-toast';
  ...
  /* ─────────────────────────────────────────────
     CALENDÁRIO
  ───────────────────────────────────────────── */
  function ag2GetAgsDoDia(dateStr) {
    var ags = window.todosAgendamentos || [];
    return ags.filter(function(a) { return a.data === dateStr; });
  }
 
  function ag2RenderCalendario() {
    var year  = ag2State.calYear;
    var month = ag2State.calMonth;
 
    // Atualiza label do mês
    var elMonth = document.getElementById('ag2-cal-month');
    if (elMonth) elMonth.textContent = fmtMesAno(year, month);
 
    var grid = document.getElementById('ag2-cal-grid');
    if (!grid) return;
 
    var firstDay = new Date(year, month, 1).getDay(); // 0=Dom
    var diasNoMes = new Date(year, month + 1, 0).getDate();
    var diasMesAnt = new Date(year, month, 0).getDate();
    var hoje = new Date().toISOString().split('T')[0];
    ...
    for (var i = firstDay - 1; i >= 0; i--) {
      var dia = diasMesAnt - i;
      var m = month === 0 ? 11 : month - 1;
      var y = month === 0 ? year - 1 : year;
      var ds = y + '-' + String(m + 1).padStart(2,'0') + '-' + String(dia).padStart(2,'0');
      html += ag2DiaHTML(dia, ds, true);
    }
 
    // Dias do mês atual
    for (var d = 1; d <= diasNoMes; d++) {
      var dateStr = year + '-' + String(month + 1).padStart(2,'0') + '-' + String(d).padStart(2,'0');
      html += ag2DiaHTML(d, dateStr, false);
    }
 
    // Completar linha final
    var total = firstDay + diasNoMes;
    var resto = total % 7 === 0 ? 0 : 7 - (total % 7);
    for (var k = 1; k <= resto; k++) {
      var m2 = month === 11 ? 0 : month + 1;
      var y2 = month === 11 ? year + 1 : year;
      var ds2 = y2 + '-' + String(m2 + 1).padStart(2,'0') + '-' + String(k).padStart(2,'0');
      html += ag2DiaHTML(k, ds2, true);
    }
 
    grid.innerHTML = html;
  }
 
  function ag2DiaHTML(diaNum, dateStr, outroMes) {
    var hoje = new Date().toISOString().split('T')[0];
    var ags  = ag2GetAgsDoDia(dateStr);
    var cls  = 'ag2-cal-day';
    if (outroMes) cls += ' outro-mes';
    if (dateStr === hoje) cls += ' hoje';
    else if (dateStr === ag2State.selectedDate) cls += ' selecionado';
 
    var dotsHTML = '';
    var temConf  = ags.some(function(a) { return a.status === 'confirmado'; });
    var temPend  = ags.some(function(a) { return a.status === 'pendente'; });
    var temCanc  = ags.some(function(a) { return a.status === 'cancelado'; });
    if (temConf) dotsHTML += '<span class="ag2-cal-dot conf"></span>';
    if (temPend) dotsHTML += '<span class="ag2-cal-dot pend"></span>';
    if (temCanc) dotsHTML += '<span class="ag2-cal-dot canc"></span>';
 
    return '<div class="' + cls + '" onclick="ag2SelecionarDia(\'' + dateStr + '\')" role="button" tabindex="0" aria-label="' + dateStr + '">' +
      '<span class="ag2-cal-day-num">' + diaNum + '</span>' +
      (dotsHTML ? '<div class="ag2-cal-day-dots">' + dotsHTML + '</div>' : '') +
    '</div>';
  }
 
  window.ag2CalPrev = function() {
    ag2State.calMonth--;
    if (ag2State.calMonth < 0) { ag2State.calMonth = 11; ag2State.calYear--; }
    ag2RenderCalendario();
  };
  window.ag2CalNext = function() {
    ag2State.calMonth++;
    if (ag2State.calMonth > 11) { ag2State.calMonth = 0; ag2State.calYear++; }
    ag2RenderCalendario();
  };
  window.ag2CalToday = function() {
    var hoje = new Date();
    ag2State.calYear  = hoje.getFullYear();
    ag2State.calMonth = hoje.getMonth();
    ag2State.selectedDate = hoje.toISOString().split('T')[0];
    ag2RenderCalendario();
    ag2RenderLista();
  };
  window.ag2SelecionarDia = function(dateStr) {
    ag2State.selectedDate = dateStr;
    ag2State.verMaisAberto = false;
    ag2RenderCalendario();
    ag2RenderLista();
  };
  ...
  /* ─────────────────────────────────────────────
     LISTA DO DIA
  ───────────────────────────────────────────── */
  var AG2_LIMITE = 5; // itens visíveis antes de "ver mais"
 
  function ag2GetAgsFiltradas() {
    var ags = window.todosAgendamentos || [];
    var data = ag2State.selectedDate;
    var lista = ags.filter(function(a) { return a.data === data; });
 
    // Filtro profissional (se implementado)
    if (ag2State.profFiltro && ag2State.profFiltro !== 'todos') {
      lista = lista.filter(function(a) { return (a.profissional || a.pacienteNome) === ag2State.profFiltro; });
    }
 
    // Ordenar
    lista.sort(function(a, b) {
      var cmp = (a.hora || '').localeCompare(b.hora || '');
      return ag2State.ordemAsc ? cmp : -cmp;
    });
 
    return lista;
  }
 
  function ag2RenderLista() {
    var lista = ag2GetAgsFiltradas();
 
    // Atualiza header
    var elDate = document.getElementById('ag2-list-date');
    if (elDate) elDate.textContent = fmtDataExtenso(ag2State.selectedDate);
 
    var elCount = document.getElementById('ag2-list-count');
    if (elCount) {
      var n = lista.length;
      elCount.textContent = n + ' agendamento' + (n !== 1 ? 's' : '');
    }
 
    var body = document.getElementById('ag2-list-body');
    if (!body) return;
 
    if (!lista.length) {
      body.innerHTML = '<div class="ag2-list-vazio">' +
        '<svg width="36" height="36" viewBox="0 0 40 40" fill="none"><rect x="5" y="6" width="30" height="28" rx="5" stroke="var(--text3)" stroke-width="1.6"/><path d="M5 14h30M13 4v6M27 4v6" stroke="var(--text3)" stroke-width="1.6" stroke-linecap="round"/></svg>' +
        '<div>Nenhum agendamento para este dia</div></div>';
      var btnVM = document.getElementById('ag2-ver-mais-btn');
      if (btnVM) btnVM.style.display = 'none';
      return;
    }
 
    var exibir = ag2State.verMaisAberto ? lista : lista.slice(0, AG2_LIMITE);
    body.innerHTML = exibir.map(ag2ItemHTML).join('');
 
    var btnVM = document.getElementById('ag2-ver-mais-btn');
    if (btnVM) {
      if (lista.length > AG2_LIMITE && !ag2State.verMaisAberto) {
        btnVM.style.display = 'flex';
        btnVM.innerHTML = '<svg width="13" height="13" viewBox="0 0 14 14" fill="none"><path d="M3 5l4 4 4-4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg> Ver mais ' + (lista.length - AG2_LIMITE) + ' agendamento' + (lista.length - AG2_LIMITE !== 1 ? 's' : '') + ' do dia';
      } else {
  ...
  function ag2ItemHTML(a) {
    var sc    = statusClassAg2(a.status);
    var label = statusLabelAg2(a.status);
    var cols  = avatarColorAg2(a.pacienteNome);
    var ini   = (a.pacienteNome || 'C')[0].toUpperCase();
    var preco = a.preco ? fmtBRL(a.preco) : '—';
    var tel   = (a.pacienteTelefone || '').replace(/\D/g,'');
    var nomeSafe = (a.pacienteNome || '').replace(/'/g,"\\'");
    var telSafe  = (a.pacienteTelefone || '').replace(/'/g,"\\'");
 
    var wppHref = tel ? 'https://wa.me/55' + tel + '?text=' + encodeURIComponent('Olá ' + (a.pacienteNome||'') + '! Confirmando seu agendamento de ' + (a.servico||'') + ' às ' + (a.hora||'') + '.') : '#';
 
    var acoesHTML = '';
    if (a.status === 'confirmado') {
      acoesHTML = '<div class="ag2-item-actions">' +
        '<a href="' + wppHref + '" target="_blank" rel="noopener" class="ag2-item-wpp-btn" title="WhatsApp" aria-label="Enviar WhatsApp">' +
        ...
        '</a>' +
        '<button class="ag2-item-more-btn" onclick="ag2ToggleDropdown(event,\'' + a._id + '\',\'' + nomeSafe + '\',\'' + telSafe + '\',\'' + (a.data||'') + '\',\'' + (a.hora||'') + '\')" type="button" aria-label="Mais opções">' +
          '<svg width="14" height="14" viewBox="0 0 15 15" fill="none"><circle cx="7.5" cy="3" r="1" fill="currentColor"/><circle cx="7.5" cy="7.5" r="1" fill="currentColor"/><circle cx="7.5" cy="12" r="1" fill="currentColor"/></svg>' +
        '</button>' +
      '</div>';
    } else if (a.status === 'pendente') {
      acoesHTML = '<div class="ag2-item-actions">' +
        (tel ? '<a href="' + wppHref + '" target="_blank" rel="noopener" class="ag2-item-wpp-btn" title="WhatsApp" aria-label="WhatsApp"><svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.123.554 4.118 1.528 5.852L.057 23.885l6.204-1.628A11.945 11.945 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.808 9.808 0 0 1-5.001-1.366l-.359-.213-3.682.966.983-3.594-.234-.371A9.818 9.818 0 0 1 2.182 12C2.182 6.57 6.57 2.182 12 2.182S21.818 6.57 21.818 12 17.43 21.818 12 21.818z"/></svg></a>' : '') +
        '<button class="ag2-item-more-btn" onclick="ag2ToggleDropdown(event,\'' + a._id + '\',\'' + nomeSafe + '\',\'' + telSafe + '\',\'' + (a.data||'') + '\',\'' + (a.hora||'') + '\')" type="button" aria-label="Mais opções">' +
          '<svg width="14" height="14" viewBox="0 0 15 15" fill="none"><circle cx="7.5" cy="3" r="1" fill="currentColor"/><circle cx="7.5" cy="7.5" r="1" fill="currentColor"/><circle cx="7.5" cy="12" r="1" fill="currentColor"/></svg>' +
        '</button>' +
      '</div>';
    ...
    return '<div class="ag2-item" id="ag2-item-' + a._id + '">' +
      '<div class="ag2-item-hora">' + (a.hora || '—') + '</div>' +
      '<div class="ag2-item-linha ' + sc + '"></div>' +
      '<div class="ag2-item-avatar" style="background:linear-gradient(135deg,' + cols[0] + ',' + cols[1] + ')">' + ini + '</div>' +
      '<div class="ag2-item-info">' +
        '<div class="ag2-item-nome">' + (a.pacienteNome || '—') + '</div>' +
        '<div class="ag2-item-servico">' + (a.servico || '—') + '</div>' +
      '</div>' +
  ...
  /* ─────────────────────────────────────────────
     DROPDOWN AÇÕES
  ───────────────────────────────────────────── */
  window.ag2ToggleDropdown = function(e, id, nome, tel, data, hora) {
    e.stopPropagation();
    // Fecha se já aberto
    if (ag2State.dropdownAberto === id) {
      ag2FecharDropdown();
      return;
    }
    ag2FecharDropdown();
    ag2State.dropdownAberto = id;
 
    var item = document.getElementById('ag2-item-' + id);
    if (!item) return;
 
    var div = document.createElement('div');
    div.className = 'ag2-item-dropdown';
    div.id = 'ag2-dropdown-' + id;
 
    var ag = (window.todosAgendamentos || []).find(function(a) { return a._id === id; }) || {};
    var st = ag.status || '';
    var wppTel = (tel || '').replace(/\D/g,'');
    var wppLink = wppTel ? 'https://wa.me/55' + wppTel : null;
 
    var btns = '';
    if (st === 'confirmado') {
      btns += '<button class="ag2-item-dropdown-btn concluir" onclick="ag2AcaoConcluir(\'' + id + '\')" type="button"><svg width="13" height="13" viewBox="0 0 15 15" fill="none"><path d="M2 7l4 4L13 4" stroke="#34d399" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg> Concluir</button>';
      btns += '<button class="ag2-item-dropdown-btn cancelar" onclick="ag2AcaoCancelar(\'' + id + '\',\'' + nome + '\',\'' + tel + '\',\'' + data + '\',\'' + hora + '\')" type="button"><svg width="13" height="13" viewBox="0 0 15 15" fill="none"><path d="M4 4l7 7M11 4l-7 7" stroke="#f87171" stroke-width="1.5" stroke-linecap="round"/></svg> Cancelar</button>';
    }
    if (st === 'pendente') {
      btns += '<button class="ag2-item-dropdown-btn concluir" onclick="ag2AcaoConfirmar(\'' + id + '\')" type="button"><svg width="13" height="13" viewBox="0 0 15 15" fill="none"><path d="M2 7l4 4L13 4" stroke="#34d399" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg> Confirmar</button>';
      btns += '<button class="ag2-item-dropdown-btn cancelar" onclick="ag2AcaoCancelar(\'' + id + '\',\'' + nome + '\',\'' + tel + '\',\'' + data + '\',\'' + hora + '\')" type="button"><svg width="13" height="13" viewBox="0 0 15 15" fill="none"><path d="M4 4l7 7M11 4l-7 7" stroke="#f87171" stroke-width="1.5" stroke-linecap="round"/></svg> Cancelar</button>';
    }
    if (wppLink) {
      btns += '<button class="ag2-item-dropdown-btn" onclick="window.open(\'' + wppLink + '\',\'_blank\')" type="button"><svg width="13" height="13" viewBox="0 0 24 24" fill="#25d366"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.123.554 4.118 1.528 5.852L.057 23.885l6.204-1.628A11.945 11.945 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.808 9.808 0 0 1-5.001-1.366l-.359-.213-3.682.966.983-3.594-.234-.371A9.818 9.818 0 0 1 2.182 12C2.182 6.57 6.57 2.182 12 2.182S21.818 6.57 21.818 12 17.43 21.818 12 21.818z"/></svg> WhatsApp</button>';
    }
    btns += '<button class="ag2-item-dropdown-btn" onclick="ag2FecharDropdown()" type="button"><svg width="13" height="13" viewBox="0 0 15 15" fill="none"><path d="M4 4l7 7M11 4l-7 7" stroke="var(--text3)" stroke-width="1.4" stroke-linecap="round"/></svg> Fechar</button>';
  ...
  function ag2FecharDropdown() {
    if (ag2State.dropdownAberto) {
      var el = document.getElementById('ag2-dropdown-' + ag2State.dropdownAberto);
      if (el) el.remove();
      ag2State.dropdownAberto = null;
    }
  }
 
  document.addEventListener('click', function(e) {
    if (ag2State.dropdownAberto && !e.target.closest('.ag2-item-dropdown') && !e.target.closest('.ag2-item-more-btn')) {
      ag2FecharDropdown();
    }
  });
  ...
  /* ─────────────────────────────────────────────
     AÇÕES
  ───────────────────────────────────────────── */
  window.ag2AcaoConcluir = function(id) {
    ag2FecharDropdown();
    if (typeof atualizar === 'function') {
      atualizar(id, 'concluido').then(function() { ag2RefreshTudo(); });
    }
  };
  window.ag2AcaoConfirmar = function(id) {
    ag2FecharDropdown();
    if (typeof atualizar === 'function') {
      atualizar(id, 'confirmado').then(function() { ag2RefreshTudo(); });
    }
  };
  window.ag2AcaoCancelar = function(id, nome, tel, data, hora) {
    ag2FecharDropdown();
    if (typeof cancelarComAviso === 'function') {
      cancelarComAviso(id, nome, tel, data, hora);
      setTimeout(ag2RefreshTudo, 800);
    }
  };
 
  window.ag2VerMais = function() {
    ag2State.verMaisAberto = true;
    ag2RenderLista();
  };
 
  window.ag2ToggleOrdem = function() {
    ag2State.ordemAsc = !ag2State.ordemAsc;
    ag2RenderLista();
  };
 
  window.ag2FiltrarProf = function(val) {
    ag2State.profFiltro = val;
    ag2RenderLista();
  };
  ...
  /* ─────────────────────────────────────────────
     STAT CARDS
  ───────────────────────────────────────────── */
  function ag2RenderStats() {
    var ags = window.todosAgendamentos || [];
    var hoje = new Date();
    var semStart = new Date(hoje); semStart.setDate(semStart.getDate() - semStart.getDay());
    var semEnd   = new Date(semStart); semEnd.setDate(semEnd.getDate() + 6);
    var semS = semStart.toISOString().split('T')[0];
    var semE = semEnd.toISOString().split('T')[0];
    ...
    set('ag2-stat-total', total);
    set('ag2-stat-conf',  conf);
    set('ag2-stat-pend',  pend);
    set('ag2-stat-canc',  canc);
  }
 
  /* ─────────────────────────────────────────────
     RESUMO DA SEMANA (sidebar)
  ───────────────────────────────────────────── */
  function ag2RenderResumoSemana() {
    var ags = window.todosAgendamentos || [];
    var hoje = new Date();
    var semStart = new Date(hoje); semStart.setDate(semStart.getDate() - semStart.getDay());
    var semEnd   = new Date(semStart); semEnd.setDate(semEnd.getDate() + 6);
    ...
    var doSem = ags.filter(function(a) { return a.data >= semS && a.data <= semE; });
    var concl = doSem.filter(function(a) { return a.status === 'concluido' || a.status === 'confirmado'; });
    var cancl  = doSem.filter(function(a) { return a.status === 'cancelado'; });
    ...
    var pctCanc = doSem.length > 0 ? Math.round((cancl.length / doSem.length) * 100) : 0;
 
    function set(id, v) { var el = document.getElementById(id); if (el) el.textContent = v; }
    set('ag2-fat-semana',   typeof window.fmtBRL === 'function' ? window.fmtBRL(fat) : 'R$ ' + fat.toFixed(2).replace('.',','));
    set('ag2-ticket-medio', typeof window.fmtBRL === 'function' ? window.fmtBRL(ticket) : 'R$ ' + ticket.toFixed(2).replace('.',','));
    set('ag2-taxa-comp',    taxa + '%');
    set('ag2-cancelamentos', cancl.length + ' (' + pctCanc + '%)');
  }
 
  /* ─────────────────────────────────────────────
     DONUT — SERVIÇOS MAIS AGENDADOS
  ───────────────────────────────────────────── */
  var AG2_DONUT_CORES = ['#3b82f6','#f59e0b','#34d399','#f87171','#a78bfa','#06b6d4','#ec4899'];
 
  function ag2RenderDonut() {
    var ags = window.todosAgendamentos || [];
    var freq = {};
    ags.forEach(function(a) {
      if (!a.servico) return;
    ...
    var outrosQtd = entries.slice(4).reduce(function(s, e) { return s + e[1]; }, 0);
    if (outrosQtd > 0) top.push(['Outros', outrosQtd]);
 
    var canvas = document.getElementById('ag2-donut-canvas');
    if (!canvas || !canvas.getContext) return;
    var ctx = canvas.getContext('2d');
    var W = canvas.width, H = canvas.height;
    var cx = W / 2, cy = H / 2, r = Math.min(W, H) / 2 - 6, inner = r * 0.55;
    ...
    top.forEach(function(e, i) {
      ...
      ctx.closePath();
      ctx.fillStyle = AG2_DONUT_CORES[i % AG2_DONUT_CORES.length];
      ctx.fill();
      startAngle += slice;
    });
    ...
    ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--bg-card') || '#111827';
    ctx.fill();
 
    // Legenda
    var legend = document.getElementById('ag2-servicos-legend');
    if (!legend) return;
    legend.innerHTML = top.map(function(e, i) {
      var pct = Math.round((e[1] / total) * 100);
      return '<div class="ag2-serv-legend-item">' +
        '<div class="ag2-serv-legend-left">' +
          '<span class="ag2-serv-legend-dot" style="background:' + AG2_DONUT_CORES[i % AG2_DONUT_CORES.length] + '"></span>' +
          '<span>' + e[0] + '</span>' +
        '</div>' +
        '<span class="ag2-serv-legend-pct">' + pct + '%</span>' +
      '</div>';
    }).join('');
  }
  ...
  /* ─────────────────────────────────────────────
     LINK DE AGENDAMENTO
  ───────────────────────────────────────────── */
  function ag2AtualizarLink() {
    var negocio = window.negocioAtual;
    if (!negocio) return;
    var url = 'https://agendorapido.com.br/agendar.html?id=' + negocio._id;
    var elTxt = document.getElementById('ag2-link-text');
    if (elTxt) elTxt.textContent = url.length > 38 ? url.slice(0, 36) + '...' : url;
  }
  ...
  window.ag2CopiarLink = function() {
    ...
    navigator.clipboard.writeText(url).then(function() {
      ag2Toast('Link copiado!', '#3b82f6');
    });
  };
 
  window.ag2CompartilharLink = function() {
    var negocio = window.negocioAtual;
    if (!negocio) return;
    var url = 'https://agendorapido.com.br/agendar.html?id=' + negocio._id;
    if (navigator.share) {
      navigator.share({ title: negocio.nome, url: url });
    } else {
      navigator.clipboard.writeText(url).then(function() {
        ag2Toast('Link copiado para compartilhar!', '#10b981');
      });
    }
  };
 
  /* ─────────────────────────────────────────────
     FILTRO PROFISSIONAL (popula select)
  ───────────────────────────────────────────── */
  function ag2PopularProfSelect() {
    var select = document.getElementById('ag2-prof-select');
    if (!select) return;
    var ags = window.todosAgendamentos || [];
    var profs = new Set();
    ags.forEach(function(a) { if (a.profissional) profs.add(a.profissional); });
  ...
  /* ─────────────────────────────────────────────
     REFRESH GERAL
  ───────────────────────────────────────────── */
  function ag2RefreshTudo() {
    ag2RenderStats();
    ag2RenderCalendario();
    ag2RenderLista();
    ag2RenderResumoSemana();
    ag2RenderDonut();
    ag2AtualizarLink();
    ag2PopularProfSelect();
  }
 
  /* ─────────────────────────────────────────────
     HOOK: intercepta navegação para "agendamentos"
  ───────────────────────────────────────────── */
  ;(function() {
    var _orig = window.irPara;
    window.irPara = function(pagina, btn) {
      _orig.apply(this, arguments);
      if (pagina === 'agendamentos') {
        setTimeout(ag2RefreshTudo, 80);
      }
    };
  })();
 
  /* ─────────────────────────────────────────────
     HOOK: atualiza ao carregar agendamentos
  ───────────────────────────────────────────── */
  ;(function() {
    var _origCarregar = window.carregarAgendamentos;
    if (typeof _origCarregar === 'function') {
      window.carregarAgendamentos = function() {
        return _origCarregar.apply(this, arguments).then
          ? _origCarregar.apply(this, arguments).then(function() { ag2RefreshTudo(); })
          : (_origCarregar.apply(this, arguments), setTimeout(ag2RefreshTudo, 300));
      };
    }
  })();
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    var check = setInterval(function() {
      tentativas++;
      if ((window.todosAgendamentos && window.todosAgendamentos.length >= 0) || tentativas > 20) {
        clearInterval(check);
        ag2RefreshTudo();
      }
    }, 300);
