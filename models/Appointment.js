
const mongoose = require('mongoose')

const appointmentSchema = new mongoose.Schema({
  clinicaId:        { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true },
  pacienteNome:     { type: String, required: true },
  pacienteTelefone: { type: String, default: '' },
  servico:          { type: String, required: true },
  preco:            { type: Number, default: 0 },
  data:             { type: String, required: true },
  hora:             { type: String, required: true },

  status: {
    type:    String,
    default: 'confirmado',
    enum:    ['confirmado', 'cancelado', 'concluido', 'aguardando_pagamento'],
  },

  pagamento: {
    status:          { type: String, enum: ['pendente', 'pago', 'reembolsado', 'rejeitado'], default: 'pendente' },
    valor:           { type: Number, default: 0 },
    paymentIntentId: { type: String, default: '' },
  },

  // ✅ NOVOS CAMPOS (opcionais, com default)
  clienteId:      { type: mongoose.Schema.Types.ObjectId, ref: 'Cliente', default: null },
  profissionalId: { type: mongoose.Schema.Types.ObjectId, ref: 'Profissional', default: null },
  observacoes:    { type: String, default: '' },
  origem:         { type: String, enum: ['publico', 'painel', 'whatsapp', 'outro'], default: 'publico' },

  criadoEm:     { type: Date, default: Date.now },
  atualizadoEm: { type: Date, default: Date.now },
})

appointmentSchema.index({ clinicaId: 1, data: 1, hora: 1 })
appointmentSchema.index({ clinicaId: 1, status: 1 })
appointmentSchema.index({ clienteId: 1 })
appointmentSchema.index({ profissionalId: 1, data: 1 })

module.exports = mongoose.model('Appointment', appointmentSchema)
